...
var app = angular.module('Qbox');
    ...
    app.controller('prepTestExamCtrl',['$mdSidenav','$interval',
      'examsTimeFactory','questionService','dataServiceFactory','$state','$mdToast','$sce','$scope','$location','$anchorScroll','baseUrl',prepTestExamCtrl]);

    function prepTestExamCtrl($mdSidenav,$interval,examsTimeFactory,questionService,dataServiceFactory,$state,$mdToast,$sce,$scope,$location,$anchorScroll,baseUrl){
...
$scope.$on('$stateChangeStart', function() {
 ...
 calibrateQuestions(dataServiceFactory.questions);
// console.log(quest);
   //  console.log(dataServiceFactory.questions);



  vm.questions = dataServiceFactory.questions;
  vm.marking = false;
 ...
 console.log(vm.questions);
  ...
  vm.viewTitle = "Prep Test : Biology";
  ...
  vm.hour = examsTimeFactory.hour
  vm.min = examsTimeFactory.min;
  vm.sec = examsTimeFactory.sec;
 ...
 var examTime =  $interval(examTimer, 1000,false);
      ...
      {  
       if(vm.hour == 0 && vm.min == 0 && vm.sec == 0){
        ...
        // console.log('answered',questionService.questionBucket);
         ...
         postResult(userID,userEduLevel,testScore,vm.questionName);
  ...
  $location.hash('marker');
   ...
   var labelArray = ['A','B','C','D','E','F','G','H','I','J','K','L','M'];  
    ...
    function postResult(userID,eduLevel,score,subject) {
      dataServiceFactory.postTestResult(userID,eduLevel,score,subject)
